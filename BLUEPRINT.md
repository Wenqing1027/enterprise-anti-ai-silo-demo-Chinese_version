...
| 旧说法 | 问题 |
|--------|------|
| 「按 Agent 技术类型做多个独立 Demo」 | 易被听成「公司有六个 Agent 产品」→ 治理对象错位 |
| 「多大脑 × 单手脚」 | 「手脚统一」对了，但「多大脑」未强调 **环由平台发版** |
| 一期硬撑 6 类环（含 Rule+LLM / Vision） | Demo 带宽被类型清单绑架；治理故事不如 4 环清晰 |
...
| 原则 | 含义 |
|------|------|
| 成品三件套 | **Loops + Tools + Skills** |
| 平台管环与工具 | 控制环与工具台账统一发版、白名单、审计口径 |
| 部门只交 Skill | 不复制环、不复制 DataFetcher、不私建工具实现 |
| 反孤岛在 L7 + Write/Govern | 语义共用、产出可订阅、能力可检索 |
| 一功能一 Skill | 一次运行一个功能；不做 Agent→Agent 对话接力 |
| 关系靠共享层不靠互聊 | flows 说明依赖；消费方另开运行读 `AIOutput`/标签 |
| 不做单 Orchestrator | 多环并存；入口永远是 `control_loop + skill_id` |
